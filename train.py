...
def get_deformable_detr_model(device):
    ...
    weight_dict.update({k + '_enc': v for k, v in clean_weight_dict.items()})
...
class DetrTransform:
    """
    Transforms PIL Images and COCO bounding boxes into the format
    expected by DETR. Includes photometric augmentations and
    bounded relative scaling during training.
    """
    ...
    def __call__(self, image, target):
        ...
        if len(normalized_boxes) > 0:
            normalized_boxes = torch.as_tensor(
                normalized_boxes, dtype=torch.float32
            )
...
def collate_fn(batch):
    ...
    Torchvision DETR handles internal padding automatically.
    """
...
def train_one_epoch(
    model, criterion, dataloader, optimizer, device, epoch, writer
):
    model.train()
    total_loss = 0.0
    progress_bar = tqdm(
        dataloader, desc=f"Epoch {epoch} [Train]", leave=False
    )
...
def validate_and_eval(
    ...
    progress_bar = tqdm(
        dataloader, desc=f"Epoch {epoch} [Valid]", leave=False
    )
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        writer.add_scalar(
            "Hyperparameters/LR_Backbone", current_lr_backbone, epoch
        )

        train_loss = train_one_epoch(
            model, criterion, train_loader, optimizer, device, epoch, writer
        )
        val_loss, current_map = validate_and_eval(
            model, criterion, valid_loader, device, epoch, writer
        )
